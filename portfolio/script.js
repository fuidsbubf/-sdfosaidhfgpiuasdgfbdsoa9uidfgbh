document.addEventListener('DOMContentLoaded', function() {
    const CONFIG = {
        DISCORD_USER_ID: '1173038204140650588',
        SPOTIFY_CLIENT_ID: '878d19e3e0634d71bdf19fb788a09832',
        DISCORD_BOT_TOKEN: null,
        UPDATE_INTERVAL: 30000,
        SPOTIFY_UPDATE_INTERVAL: 5000,
        EMAIL: 'No email atm, just dm me on discord' // Translated from 'votre-vraie-adresse@gmail.com'
    };
    ...
    async function updateDiscordStatus() {
        ...
        try {
            ...
            if (lookupResponse.ok) {
                ...
                console.log('✅ Discord Lookup data received:', lookupData);
            ...
            if (discordResponse.ok) {
                ...
                console.log('✅ Discord API data received:', discordData);
                updateDiscordUIFromAPI(discordData);
                return;
            }
            
        } catch (error) {
            console.error('❌ Error retrieving Discord data:', error);
        }
                ...
                if (userData.activities && userData.activities.length > 0) {
                    ...
                    if (filteredActivities.length > 0) {
                        ...
                        switch (currentActivity.type) {
                            case 0:
                                activityText = `🎮 Playing ${currentActivity.name}`;
                                ...
                                break;
                            case 1:
                                activityText = `🔴 Streaming ${currentActivity.name}`;
                                if (activityImage) activityImage.style.display = 'none';
                                break;
                            case 2:
                                activityText = `🎵 Listening to ${currentActivity.name}`;
                                if (activityImage) activityImage.style.display = 'none';
                                break;
                            case 3:
                                activityText = `📺 Watching ${currentActivity.name}`;
                                if (activityImage) activityImage.style.display = 'none';
                                break;
                            default:
    ...
    function updateDiscordOffline() {
        console.log('😴 Discord offline mode');
    ...
    function copyDiscord() {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(username).then(() => {
                showNotification(`Discord username copied: ${username}`);
            }).catch(err => {
                console.error('Clipboard error:', err);
                showNotification('❌ Error copying');
            });
            ...
            try {
                document.execCommand('copy');
                showNotification(`Discord copied: ${username}`);
            } catch (err) {
                showNotification('❌ Error copying');
            }
            document.body.removeChild(textArea);
...
function copyDiscordUsername() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(currentUsername).then(() => {
            showNotification(`Discord copied: ${currentUsername}`);
        }).catch(() => {
...
function fallbackCopyTextToClipboard(text, type) {
    ...
    try {
        ...
        if (successful) {
            showNotification(`${type} copied: ${text}`);
        } else {
            showNotification(`Error copying ${type.toLowerCase()}`);
        }
    } catch (err) {
        showNotification(`Error copying ${type.toLowerCase()}`);
    }
